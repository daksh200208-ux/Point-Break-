...
def _split_into_sentences(text: str):
    """Splits text into natural conversational sentence chunks optimized for low-latency TTS pipelining."""
    raw_chunks = re.split(r'(?<=[.!?;])\s+|\n+', text)
    valid_chunks = [c.strip() for c in raw_chunks if c.strip()]
    if not valid_chunks:
        return [text.strip()]

    # Merge short fragments into natural breath groups (8-word minimum keeps CPU synthesis fast ~1-2s per chunk)
    merged = []
    buf = ""
    for c in valid_chunks:
        if buf:
            ...
            buf = c
        if len(buf.split()) >= 8 or buf.endswith(("!", "?")):
            merged.append(buf)
            buf = ""
    if buf:
        if merged:
...
def speech_worker():
    ...
    async def gen_audio(raw_text, pitch, rate, vol, out_path):
        ...
        try:
            ...
            res = generate_tars_audio(cleaned_text, wav_target)
    ...
    while True:
        ...
        try:
            ...
            while idx < len(sentences):
                ...
                # ── Lookahead Pipelining: check if sentence was pre-generated by previous iteration ──
                if hasattr(speech_worker, '_lookahead_result') and speech_worker._lookahead_result is not None:
                    _la_idx, _la_path = speech_worker._lookahead_result
                    speech_worker._lookahead_result = None
                    if _la_idx == idx and _la_path and os.path.exists(_la_path) and os.path.getsize(_la_path) > 0:
                        generated_audio_path = _la_path
                        spoke_online = True

                # If not pre-generated (first sentence or lookahead missed), generate synchronously
                if not spoke_online:
                ...
                if hard_interrupted:
                    ...
                    # Clean up any pending lookahead
                    if hasattr(speech_worker, '_lookahead_future') and speech_worker._lookahead_future is not None:
                        speech_worker._lookahead_future = None
                    if hasattr(speech_worker, '_lookahead_result') and speech_worker._lookahead_result is not None:
                        try:
                            _, _la_p = speech_worker._lookahead_result
                            if _la_p and os.path.exists(_la_p): os.remove(_la_p)
                        except: pass
                        speech_worker._lookahead_result = None
                    break
                ...
                if spoke_online and generated_audio_path:
                    ...
                    # ── Lookahead: Pre-generate next sentence in background while current one plays ──
                    _next_idx = idx + 1
                    speech_worker._lookahead_future = None
                    if _next_idx < len(sentences) and not hard_interrupted:
                        _next_sentence = sentences[_next_idx]
                        if resume_prefix and not _next_sentence.lower().startswith("as i was"):
                            _next_sentence = resume_prefix + _next_sentence
                        _la_fd, _la_tmp = tempfile.mkstemp(suffix=".wav", dir=JARVIS_DIR)
                        os.close(_la_fd)
                        def _pre_gen_next(_s, _p, _pitch, _rate, _vol, _target_idx):
                            """Pre-generate next sentence audio during current playback."""
                            try:
                                _la_loop = asyncio.new_event_loop()
                                _result = _la_loop.run_until_complete(gen_audio(_s, _pitch, _rate, _vol, _p))
                                _la_loop.close()
                                if _result and os.path.exists(_result) and os.path.getsize(_result) > 0:
                                    speech_worker._lookahead_result = (_target_idx, _result)
                                else:
                                    speech_worker._lookahead_result = None
                            except Exception:
                                speech_worker._lookahead_result = None
                        speech_worker._lookahead_result = None
                        _la_thread = threading.Thread(
                            target=_pre_gen_next,
                            args=(_next_sentence, _la_tmp, TARS_NORMAL_PITCH, TARS_NORMAL_RATE, TARS_NORMAL_VOL, _next_idx),
                            daemon=True
                        )
                        _la_thread.start()
                        speech_worker._lookahead_future = _la_thread

                    completed = play_chunk(generated_audio_path)

                    # Wait for lookahead thread to finish if it was started (max 8s safety cap)
                    if speech_worker._lookahead_future is not None:
                        speech_worker._lookahead_future.join(timeout=8.0)
                        speech_worker._lookahead_future = None

                    if hard_interrupted:
                        # Clean up pre-generated audio on interrupt
                        if hasattr(speech_worker, '_lookahead_result') and speech_worker._lookahead_result is not None:
                            try:
                                _, _la_p = speech_worker._lookahead_result
                                if _la_p and os.path.exists(_la_p): os.remove(_la_p)
                        ...
                        break
                    if verbal_interrupted:
                        # Clean up pre-generated audio on verbal interrupt
                        if hasattr(speech_worker, '_lookahead_result') and speech_worker._lookahead_result is not None:
                            try:
                                _, _la_p = speech_worker._lookahead_result
                                if _la_p and os.path.exists(_la_p): os.remove(_la_p)
                    ...
                    # Natural cadence pause between sentences (0.3s — keeps rhythm crisp without dead time)
                    if _next_idx < len(sentences):
                        time.sleep(0.3)
...
def take_command(timeout=None):
    ...
    # Acoustic Clearance: Do NOT listen while Point Break is speaking, handling an interruption, or immediately after (0.30s reverb window)
    if tars_speaking or in_interruption_handling or (time.time() - _last_spoken_finish_time < 0.30):
        time.sleep(0.1)
        return "none"
...
def _extract_message(raw: str) -> str:
    # "post/share/publish [about] <msg> on/to/in <platform>" pattern
    m_post = re.search(r'(?:post|publish|share|write|draft)\s+(?:about|regarding)?\s*["\']?(.+?)["\']?\s+(?:on|to|in)\s+(?:linkedin|twitter|x|facebook|reddit|instagram|threads)', raw, re.IGNORECASE)
    if m_post:
        extracted = m_post.group(1).strip()
        if any(w in extracted.lower() for w in ["himself", "yourself", "itself", "point break", "tars", "who you are"]):
            return "Excited to introduce Point Break — an autonomous AI desktop assistant designed to execute complex workflows, voice interactions, and visual GUI automation directly on Windows."
        if extracted and len(extracted) > 1:
            return extracted

    # "post/share on <platform> [saying/about] <msg>" pattern
    m_post2 = re.search(r'(?:post|publish|share|write|draft)\s+(?:on|to|in)\s+\w+[:\s]+(?:about|that|saying)?\s*["\']?(.+?)["\']?$', raw, re.IGNORECASE)
    if m_post2:
        extracted = m_post2.group(1).strip()
        if extracted and len(extracted) > 1:
            return extracted
    ...
    return "Excited to introduce Point Break — an autonomous AI desktop assistant."
...
def expand_gui_objective(raw: str) -> str:
    ...
    # ── LinkedIn Post / Publish / Comment ───────────────────────────────────
    if "linkedin" in low:
        if any(k in low for k in ["post", "publish", "share", "draft", "write"]):
            msg_text = _extract_message(raw)
            if not msg_text or msg_text == raw:
                msg_text = "DRAFT_PLACEHOLDER"
            return (
                f"Post on LinkedIn: '{msg_text}'. "
                f"STEP 1: Press Win key, type 'Chrome', press Enter. "
                f"STEP 2: Click the address bar, type 'linkedin.com/feed', press Enter. "
                f"STEP 3: Wait for the LinkedIn feed to fully load. "
                f"STEP 4: Click the 'Start a post' button or the post creation text area. "
                f"STEP 5: Wait for the post editor modal to appear. "
                f"STEP 6: Type the post content into the editor. "
                f"STEP 7: STOP. Inform the user: 'Sir, I have drafted your LinkedIn post. It is on your screen for review. Please confirm if you want me to post it, or if you would like adjustments.' Mark done."
            )
        elif any(k in low for k in ["message", "dm", "send"]):
            person = _extract_person(raw)
            msg_text = _extract_message(raw)
            return (
                f"Send LinkedIn message to {person}: '{msg_text}'. "
                f"STEP 1: Press Win key, type 'Chrome', press Enter. "
                f"STEP 2: Click address bar, type 'linkedin.com/messaging', press Enter. "
                f"STEP 3: Click the compose/new message icon. "
                f"STEP 4: Type '{person}' in the recipient search, click the matching profile. "
                f"STEP 5: Click the message input box. "
                f"STEP 6: Type '{msg_text}'. "
            ...
            return (
                f"Open LinkedIn. "
                f"STEP 1: Press Win key, type 'Chrome', press Enter. "
                f"STEP 2: Click address bar, type 'linkedin.com', press Enter. Mark done."
            )
    ...
    try:
        _gui_expand_prompt = (
            ...
            f"User wants: \"{raw}\"\n\n"
            f"Create a safe, precise step-by-step GUI action plan to accomplish this task.\n"
            f"Rules:\n"
            f"- Use STEP N: format for each step.\n"
            f"- Steps can be: press keys, click UI elements, type text, navigate URLs, wait for loading.\n"
            f"- For irreversible actions (posting, sending, purchasing, deleting), the LAST step MUST be:\n"
            f"  'STOP. Inform the user for review and confirmation before proceeding.'\n"
            f"- Keep it to 3-8 steps maximum.\n"
            f"- Be specific about what to click/type, not vague.\n\n"
            f"Output ONLY the step plan, nothing else."
        )
        _expand_res = query_generative_model('gemini-3.5-flash-lite', _gui_expand_prompt, timeout=10.0)
        if _expand_res and "STEP" in _expand_res.upper():
            print(f"[GUI Agent] Dynamic objective expansion for: '{raw[:60]}'")
            return f"{raw}. {_expand_res}"
    except Exception as _exp_err:
        print(f"[GUI Agent] Dynamic expansion failed: {_exp_err}")

    # Final fallback: cannot expand this objective safely
    return ""
...
def execute_gui_agent_flow(objective: str):
    ...
    print(f"[GUI] Executing {len(steps)} steps for: {objective[:80]}...")
    speak("Initiating desktop automation sequence, sir.", block=False)
    ...
    for i, step_text in enumerate(steps):
        ...
        if any(trigger in step_lower for trigger in ["stop", "inform the user", "review", "confirm", "please confirm", "ask the user", "wait for confirmation"]):
            ...
            if "inform the user:" in step_lower:
                ...
                _review_msg = step_text.split(":", 1)[-1].strip().strip("'\"")
            if not _review_msg or len(_review_msg) < 5:
                _review_msg = "Sir, I have completed the preparation. The result is on your screen for review. Please confirm if you would like me to proceed."
        ...
        # ── Handle keyboard-only steps (press Win, press Enter, hotkeys) ──
        _key_match = re.search(r'\b(?:press|hit|tap)\s+(.+?)(?:\.|,|$)', step_text, re.I)
        if _key_match and not any(k in step_lower for k in ["click", "type", "navigate", "wait", "open"]):
            _keys_raw = _key_match.group(1).strip().lower()
            _key_map = {
                "win": "win", "windows": "win", "win key": "win", "windows key": "win",
            ...
            if "+" in _keys_raw:
                ...
                try:
                    pyautogui.hotkey(*_mapped)
                    print(f"[GUI]   -> Hotkey: {'+'.join(_mapped)}")
                except Exception as _ke:
                    print(f"[GUI]   -> Hotkey failed: {_ke}")
            else:
                _single = _key_map.get(_keys_raw, _keys_raw)
                try:
                    pyautogui.press(_single)
                    print(f"[GUI]   -> Key press: {_single}")
                except Exception as _ke:
                    print(f"[GUI]   -> Key press failed: {_ke}")
            time.sleep(0.8)
            continue
        ...
        # ── Vision-guided step execution ──
        img_bytes = capture_desktop_screenshot()
        if not img_bytes:
            speak("Screen capture failed, sir. Unable to proceed.")
            break

        step_data = query_gui_agent_step(img_bytes, step_text)
        if not step_data:
            print(f"[GUI] No response for step {i+1}, attempting to continue.")
            time.sleep(0.5)
            continue
        ...
        try:
            ...
            if action == "click":
                ...
                _hk_parts = [k.strip() for k in str(text).split("+")]
                pyautogui.hotkey(*_hk_parts)
            ...
            speak(f"Step {i+1} encountered an error, sir. Attempting to continue.")
            continue
    else:
        speak("All steps completed successfully, sir.")
...
def _execute_single(query: str):
    ...
    # ── SMART GUI ACTION INTERCEPTOR (Vision-Guided Desktop Automation) ─────
    # Catches operational commands that require GUI interaction: social posting, form filling, app navigation
    _gui_action_patterns = [
        r'\b(?:post|publish|share|draft)\b.*\b(?:on|to|in)\b.*\b(?:linkedin|twitter|x\.com|facebook|reddit|instagram|threads)\b',
        r'\b(?:linkedin|twitter|facebook|reddit|instagram)\b.*\b(?:post|publish|share|draft|write|update)\b',
        r'\b(?:fill|submit|complete)\b.*\b(?:form|application|survey)\b',
        r'\baction\s+(?:click|type|gui|press)\b',
        r'\bgui\s+(?:action|execute|automate)\b',
        r'\b(?:create|make|write)\b.*\b(?:a\s+post|an?\s+update|a\s+tweet|a\s+status)\b',
    ]
    ...
    if _is_gui_action:
        ...
        if _gui_objective:
            ...
            print("  [GUI Agent] Could not expand objective, falling through to AI router.")
    ...
    if action_data:
        try:
            ...
            elif action in ["post_social", "social_post", "create_post"]:
                # Direct social media posting action from Gemini AI
                platform = action_data.get("platform", "").lower() or arg.split()[0] if arg else ""
                content = action_data.get("content", "") or action_data.get("body", "") or arg or query
                _post_query = f"post on {platform}: {content}" if platform else f"post: {content}"
